...
class FoodMenuCleanupSettings(Base):
    ...
    # NULL means auto-delete is turned off for this chat. Deliberately no `default=`/`server_default`
    # here: SQLAlchemy's column default fires whenever the value is None — including an explicit
    ...
    # repository's `get_or_create`.
    delay_minutes: Mapped[int | None] = mapped_column(SmallInteger)
